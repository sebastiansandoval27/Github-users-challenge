...
const customSpan = (text: string) => {
  return (
    <span className='text-2xl font-bold text-cyan-800 mr-2'>{text}:</span>
  )
}
  ...
  return (
    <Layout>
      {loading ? (
        ...
        <div className="content flex flex-col justify-start items-start gap-3"
          style={{ height: `calc(100vh - 12.5rem)` }}
          id="User"
        >
          <h2 className='text-2xl text-white w-full bg-cyan-700 py-3 px-5'>User Details with the Id  # <span className='font-black'>
            {user.id}</span></h2>
          <div className='userGrid w-full border-2 border-cyan-700'>
            ...
            <div className='grid-followers flex justify-start w-full'>
              {customSpan('Followers / following')} <h1 className='text-2xl'>{user.followers} / {user.following}</h1>
            </div>
